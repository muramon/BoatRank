...
class BackTest():
    ...
    def backtest(self, test_ds=None, model=None, odds_json=None, model_type='RankNet', test_file = 'dataset/Toda_191201-191231.txt', kaijo=None, three=False):
        ...
        if three:
            ...
            csv_file = "./tansho_thresh_test/long_backtest_{}_model{}_{}.csv".format(model_type, model_name, test_file[8:-4])
        ...
        for qid, batch_rankings, labels in test_ds:
            ...
            try:
                ...
                for i in range(20):
                    win[i] = 0
                    if int(odds[0][1]) >= (50 * (i + 2)) and int(odds[0][1]) < (50 * (i + 3)):
                        win[i] = -100
                        count[i] += 1
