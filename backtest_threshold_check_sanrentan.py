...
class BackTest():
    ...
    def backtest(self, test_ds=None, model=None, odds_json=None, model_type='RankNet', test_file = 'dataset/Toda_191201-191231.txt', kaijo=None, three=False):
        ...
        if three:
            ...
            csv_file = "./sanrentan_thresh_test/long_backtest_{}_model{}_{}.csv".format(model_type, model_name, test_file[8:-4])
            ...
            try:
                ...
                for i in range(100):
                    ...
                    if int(odds[8][1]) >= (100 * (i + 2)) and int(odds[8][1]) < (100 * (i + 3)):
                        ...
                        i += 1
                if int(odds[8][1]) >= 10200:
                    win[100] = -100
                    count[i] += 1
                    if pred_rank[0] == int(odds[8][0][0]) and pred_rank[1] == int(odds[8][0][2]) and pred_rank[2] == int(odds[8][0][4]):
                        win[100] += int(odds[8][1])
                    win_total[100] += win[100]
                else:
                    win[100] = 0
                    win_total[100] += 0
